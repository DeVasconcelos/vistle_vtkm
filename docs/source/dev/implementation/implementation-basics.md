...
Implementing read modules is very similar to implementing compute modules: their output ports and parameters are defined in the same manner.
However, read modules inherit from the `vistle::Reader` instead of the `vistle::Module` class. Moreover, the main work happens in the `read` instead of the `compute` function.
